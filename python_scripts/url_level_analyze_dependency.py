import codecs, json, re, time, os, getopt, traceback#, hashlib
...
def get_both_scripts(script2reads, first_party_domain, static_id2url, priv2scripts):
    both_scripts = list()
    first_scripts = [s for s in priv2scripts['1']]
    third_scripts = [s for s in priv2scripts['3']]
    script2priv = dict()
    for priv, scripts in list(priv2scripts.items()):
        for script_info in scripts:
            if script_info not in script2priv:
                script2priv[script_info] = priv
    for script in third_scripts:
        if script not in script2reads:
            continue
        read_priv_list = [script2priv[read] for read in script2reads[script]]
        if len(set(read_priv_list)) > 1 and 1 in read_priv_list:
            # 3p script required (read) by both 1p and 3p scripts:
    ...
    for script in first_scripts:
        ...
        read_priv_list = [script2priv[read] for read in script2reads[script]]
        if len(set(read_priv_list)) > 1 and 1 in read_priv_list:
            # 1p script required (read) by both 1p and 3p scripts:
            both_scripts.append(script)
...
def measure(user_dir, task_id, length, start, end, status_queue, process_index):
    global processed_data_dir, extract, rank2domain, domain2revdomains
    ...
    try:
                            ...
                            for line in input_f:
                                try:
                                    ...
                                    if line_split[0] == '[R]':
                                        ...
                                        # currently we only have 2 types of logs, [R]/[W]
                                        pass
        ...
        for rank, url2configs in list(rank2url2configs.items()):
            ...
            for url, configs in list(corrected_url2configs.items()):
                ...
                for config in configs:
                    script_url = config['match']
                    simple_url2configs[url][script_url] = config['world_id']
...
def main(argv):
    global raw_data_dir, processed_data_dir, num_instances, parent_pid, process_list, log_f, extract, rank2domain, domain2revdomains
    ...
    # comment the following 2 lines if you want to generate policies without using the grount truth
    with open('domain2revlist.json', 'r') as input_f:
        domain2revdomains = json.loads(input_f.read())

    raw_data_dir = exp_dir
    processed_data_dir = os.path.join(exp_dir, 'url-level-policies')
