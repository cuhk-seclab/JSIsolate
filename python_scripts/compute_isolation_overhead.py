...
def measure(user_dir, task_id, start, end):
    ...
    iso_time_files = [f for f in files if f.endswith('.time_isolate')]
    mem_files = [f for f in files if f.endswith('.mem')]
    iso_mem_files = [f for f in files if f.endswith('.mem_isolate')]
...
def main(argv):
    ...
    output_file = 'type2rank2time.json'
    output_file = os.path.join(output_dir, output_file)
    with open(output_file, 'w') as output_f:
        output_f.write(json.dumps(type2rank2time))
        
    output_file = 'type2rank2mem.json'
    output_file = os.path.join(output_dir, output_file)
    with open(output_file, 'w') as output_f:
    ...
    for key in list(clean_times.keys()):
        ...
        isolate_mean_time = sum(cur_isolate_times_) / (max(len(cur_isolate_times_), 1) * 1.0)
